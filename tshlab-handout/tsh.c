 ...
 */
void eval_out(struct cmdline_tokens tok,int bg,char* cmdline){
    sigset_t mask_a,mask_p;
    sigfillset(&mask_a);
    sigprocmask(SIG_BLOCK, &mask_a, &mask_p);//阻塞所有信号，避免在创建子进程与addjob间存在竞争
    pid_t pid;
    pid=fork();
    if(pid==0){
        setpgid(0,0);
        sigprocmask(SIG_SETMASK, &mask_p, NULL);
...
void eval_k(struct cmdline_tokens tok,int bg,char* cmdline){
    struct job_t *job;
    pid_t pid;
    int jid;
    if(tok.argv[1][0]=='%'){
...
void eval_b(struct cmdline_tokens tok,int bg,char* cmdline){
    struct job_t *job;
    pid_t pid;
    int jid;
    if(tok.argv[1][0]=='%'){
...
void eval_f(struct cmdline_tokens tok,int bg,char* cmdline){
    struct job_t *job;
    pid_t pid;
    int jid;
    if(tok.argv[1][0]=='%'){
...
void eval_nohup(struct cmdline_tokens tok,int bg,char* cmdline){
    ...
    sigemptyset(&mh);
    sigaddset(&mh,SIGHUP);
    sigprocmask(SIG_BLOCK, &mh, &mp);
    char *a=cmdline+6;
    eval(a);
    sigprocmask(SIG_SETMASK, &mp, NULL);
}
void 
eval(char *cmdline) 
{
    ...
    ou_f=STDOUT_FILENO;
    if(tok.infile){
        in_f=open(tok.infile,O_RDONLY,0);
    }
    if(tok.outfile){
        ou_f=open(tok.outfile,O_WRONLY,0);
    }
    int stdi=dup(STDIN_FILENO);
    int stdo=dup(STDOUT_FILENO);
    dup2(in_f, STDIN_FILENO);
    dup2(ou_f, STDOUT_FILENO);
    switch(tok.builtins){
        case BUILTIN_NONE:
            eval_out(tok,bg,cmdline);
            break;
        case BUILTIN_BG:
            eval_b(tok,bg,cmdline);
            break;
        case BUILTIN_FG:
            eval_f(tok,bg,cmdline);
            break;
        case BUILTIN_JOBS:
            listjobs(job_list, ou_f);
            break;
        case BUILTIN_KILL:
            eval_k(tok,bg,cmdline);
            break;
        case BUILTIN_NOHUP:
...
sigchld_handler(int sig) 
{
    ...
    sigset_t ma,mp;
    sigfillset(&ma); 
    while ((pid = waitpid(-1, &s, WNOHANG | WUNTRACED | WCONTINUED)) > 0){
        job=getjobpid(job_list,pid);
        if(WIFEXITED(s)){
            sigprocmask(SIG_SETMASK, &ma, &mp);
            deletejob(job_list, pid);
            sigprocmask(SIG_SETMASK, &mp, NULL);
        }
        if (WIFSIGNALED(s)) {
            sio_put("Job [%d] (%d) terminated by signal %d\n", job->jid, pid, WTERMSIG(s));
            sigprocmask(SIG_SETMASK, &ma, &mp);
            deletejob(job_list, pid);
            sigprocmask(SIG_SETMASK, &mp, NULL);
        }
        if (WIFSTOPPED(s)) {
            sio_put("Job [%d] (%d) stopped by signal %d\n", job->jid, pid, WSTOPSIG(s));
            sigprocmask(SIG_SETMASK, &ma, &mp);
            job->state = ST;
            sigprocmask(SIG_SETMASK, &mp, NULL);
        }
        if (WIFCONTINUED(s) && job->state == ST) {
            sigprocmask(SIG_SETMASK, &ma, &mp);
            job->state = BG;
            sigprocmask(SIG_SETMASK, &mp, NULL);
        }
